"""Defines the QAM16Config data class for use with QAM16()."""
...
class QAM16Config(ModemConfig):
    """Dataclass for use with the QAM16() ctor."""

    # ATTRIBUTES
    # Public

    carrier_recovery: CostasLoop | None = field(default=None)  # Carrier recovery obj
    mapper: dict[int, complex] | None = None                   # Bit mapping (defaults to QAM16_MAP)

    # Private

    _bits_per_sym = 4                                          # Bits per symbol
    ...
    def validate_content(self) -> None:
        ...
        if self._validated is not True:
            self.validate_qam16()  # Validate the 16QAM-specific data
