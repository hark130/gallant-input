"""Defines the base modem.qam16 QAM16() Component Test Class.
...
Typical Usage:
    ...
    python -m test.comp_test.test_modem.test_modem_qam16  # Run *all* QAM16 test cases
    # Run just this normal 1 unit test
    python -m test.comp_test.test_modem.test_modem_qam16 -k n01
"""
...
from test.modify import add_awgn, convert_bin_bytes_to_qam16, generate_bin_bytes, rotate_mapping


# pylint: disable=too-many-instance-attributes
# Leave me be, Pylint
class QAM16ModemCompTest(ModemCompTest):
    """GAIN.modem.qam16 QAM16() component test class.

    Defines functionality needed to run component tests on the QAM16() modem class.
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        """QAM16ModemCompTest ctor."""
        super().__init__(*args, **kwargs)

        # ATTRIBUTES
        self.input_carrier_recovery = None  # Optional QAM16Config attribute
        self.input_mapper = None            # Optional QAM16Config attribute
        self._demod_arg_s = None            # Test case input: QAM16().demodulate(samples) arg
        self._demod_arg_f = None            # Test case input: QAM16().demodulate(filt) arg
        self._demod_arg_strat = None        # Test case input: QAM16().demodulate(strategy) arg
        self._mod_arg_bb = None             # Test case input: QAM16().modulate(bin_bytes) arg
    ...
    def run_test_exception(self, exception_type: Exception, exception_msg: str,
                           bin_bytes: Any, samples: Any, filt: Any, strategy: Any,
                           modem_order: bool = True) -> None:
        """Common method calls for a test case expected to raise an exception.

        Test author must call self.set_qam16_ctor_args() *and* self.set_test_input().
        ...
        """
        self.set_oob_test_input(bin_bytes, samples, filt, strategy, modem_order)
    ...
    def run_test_return_input(self, bin_bytes: Any, samples: Any, filt: Any,
                              strategy: DecideSymbols, modem_order: bool = True) -> None:
        ...
        Args:
            ...
            samples: Test case input for the demodulate method argument of the same name.
            filt: Test case input for the demodulate method argument of the same name.
            strategy: The demodulate(symbol_strategy) argument.
            modem_order: [OPTIONAL] If True, the test case will call modulate() then demodulate().
                Othersise, the call order is reversed.
        """
        self.set_test_input_return(bin_bytes=bin_bytes, samples=samples, filt=filt,
                                   strategy=strategy, modem_order=modem_order)
        self.run_test()

    def run_test_return_noisy_input(self, bin_bytes: Any, samples: Any, filt: Any,
                                    strategy: DecideSymbols, snr_db: float | int,
                                    modem_order: bool = False) -> None:
        """Common method for a test case expected to return an expected result on noisy input.

        The expected results depends on modem_order.  Test author must first call
        self.set_qam16_ctor_args().  The modem order will always be False and bin_bytes will
        ...
        self.set_test_input_return(bin_bytes=bin_bytes, samples=noisy, filt=filt,
                                   strategy=strategy, modem_order=modem_order, skip_exp_ret=True)
    ...
    def set_demodulate_test_input(self, samples: Any, filt: Any, strategy: Any) -> None:
        ...
        self._demod_arg_strat = strategy
    ...
    def set_oob_test_input(self, bin_bytes: Any, samples: Any,
                           filt: Any, strategy: DecideSymbols, modem_order: bool = True) -> None:
        ...
        self.set_demodulate_test_input(samples=samples, filt=filt, strategy=strategy)
        self._modem_call_order = modem_order

    # pylint: disable=too-many-arguments,too-many-positional-arguments
    def set_test_input_return(self, bin_bytes: Any, samples: Any, filt: Any,
                              strategy: DecideSymbols, modem_order: bool = True,
                              skip_exp_ret: bool = False) -> None:
    ...
    def create_test_obj(self) -> QAM16:
        ...
        Strongly consider calling self.set_modem_ctor_args() and self.set_qam16_ctor_args() first.
        """
...
class NormalQAM16ModemCompTest(QAM16ModemCompTest):
    """Normal Test Cases."""

    def test_n01_quad_word_random_bits_mo_dem(self):
        ...
        self.run_test_return_input(bin_bytes, samples, filt, strategy, modem_order=True)

    def test_n02_single_word_random_bits_dem_mo(self):
        """Eight bytes of random bits, dem --> mo order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
        mapper = QAM16_MAP  # Used by test code "double do" helper function
        # modulate()/demodulate() args
        bin_bytes = generate_bin_bytes(num_bits=8*8)
        samples = convert_bin_bytes_to_qam16(bin_bytes, samp_rate, sym_rate, mapper)
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.NEAR  # Default value
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.run_test_return_input(bin_bytes, samples, filt, strategy, modem_order=False)

    def test_n03_random_bits_unfiltered_mo_dem_strategy_axis(self):
        """Random bits, no matched filter, decide symbols strategy: axis, mo --> dem order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
        mapper = None  # Defaults to QAM16_MAP
        # modulate()/demodulate() args
        bin_bytes = generate_bin_bytes(num_bits=1024)
        samples = None  # Will be defined by dynamic test case execution
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.AXIS
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.run_test_return_input(bin_bytes, samples, filt, strategy, modem_order=True)

    def test_n04_random_bits_unfiltered_dem_mo_strategy_axis(self):
        """Random bits, no matched filter, decide symbols strategy: axis, dem --> mo order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
    ...
    def test_n05_random_bits_unfiltered_mo_dem_strategy_kmeans(self):
        """Random bits, no matched filter, decide symbols strategy: kmeans, mo --> dem order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
        ...
        samples = None  # Will be defined by dynamic test case execution
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.KMEANS
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.run_test_return_input(bin_bytes, samples, filt, strategy, modem_order=True)

    def test_n06_random_bits_unfiltered_dem_mo_strategy_kmeans(self):
        """Random bits, no matched filter, decide symbols strategy: kmeans, dem --> mo order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
    ...
    def test_n07_random_bits_unfiltered_mo_dem_strategy_kmeans_gain(self):
        """Random bits, no matched filter, decide symbols strat: kmeans gains, mo --> dem order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
        ...
        samples = None  # Will be defined by dynamic test case execution
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.KMEANS_GAIN
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.run_test_return_input(bin_bytes, samples, filt, strategy, modem_order=True)

    def test_n08_random_bits_unfiltered_dem_mo_strategy_kmeans_gain(self):
        """Random bits, no matched filter, decide symbols strat: kmeans gains, dem --> mo order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
    ...
    def test_n09_random_bits_unfiltered_mo_dem_strategy_near(self):
        """Random bits, no matched filter, decide symbols strategy: near, mo --> dem order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
    ...
    def test_n10_random_bits_unfiltered_dem_mo_strategy_near(self):
        """Random bits, no matched filter, decide symbols strategy: near, dem --> mo order."""
        # QAM16Config() args
        samp_rate = 4800
        sym_rate = 80
        carr_rec = None
    ...
    def test_n11_random_bits_noisy_mo_dem(self):
        ...
        self.run_test_return_noisy_input(bin_bytes=bin_bytes, samples=samples, filt=filt,
                                         strategy=strategy, snr_db=self.SNR_POOR, modem_order=True)
    ...
    def test_n12_random_bits_noisy_dem_mo(self):
        ...
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.run_test_return_noisy_input(bin_bytes=bin_bytes, samples=samples, filt=filt,
                                         strategy=strategy, snr_db=self.SNR_POOR, modem_order=False)


class BoundaryQAM16ModemCompTest(QAM16ModemCompTest):
    """Boundary Test Cases."""
    ...
    def test_b07_samples_per_symbol_too_small_mo_dem(self):
        ...
        self.run_test_exception(ValueError, 'argument is not positive',
                                bin_bytes, samples, filt, strategy, modem_order=True)
    ...
    def test_b08_samples_per_symbol_too_small_dem_mo(self):
        ...
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.run_test_exception(ValueError, 'argument is not positive',
                                bin_bytes, samples, filt, strategy, modem_order=False)


class SpecialQAM16ModemCompTest(QAM16ModemCompTest):
    """Special Test Cases."""
    ...
    def test_s04_weird_mapper_rotated_90_deg_mo_dem(self):
        ...
        sym_rate = 800
        carr_rec = None
        mapper = rotate_mapping(QAM16_MAP, numpy.pi / 2)
        # modulate()/demodulate() args
        bin_bytes = generate_bin_bytes(num_bits=256)
        samples = None  # Will be defined by dynamic test case execution
        filt = MatchedFilter.NONE
    ...
    def test_s33_trailing_zero_needs_padding_mo_dem(self):
        ...
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.set_test_input_return(bin_bytes=bin_bytes, samples=samples, filt=filt,
                                   strategy=strategy, modem_order=True, skip_exp_ret=True)
    ...
    def test_s35_odd_binary_len_leading_zero_de_mod(self):
        ...
        self.expect_return(bin_bytes + b'0')  # QAM16 output is always even
