...
def convert_bin_bytes_to_bpsk(bin_bytes: bytes, sample_rate: int | float, symbol_rate: int | float,
    ...
    # INPUT VALIDATION
    validate_mapper(bit_map, 'bit_map', bits_per_symbol=bps)  # B is for Binary

    # COMPUTE IT
    array = _convert_bin_bytes_to_mod(bin_bytes=bin_bytes, samples_per_symbol=sps,
                                      bit_map=bit_map, bits_per_symbol=bps)
...
def convert_bin_bytes_to_qam16(bin_bytes: bytes, sample_rate: int | float, symbol_rate: int | float,
                               bit_map: dict[int, complex]) -> numpy.ndarray:
    ...
    # INPUT VALIDATION
    validate_mapper(bit_map, 'bit_map', bits_per_symbol=bps)  # 16-QAM
...
def _convert_bin_bytes_to_mod(bin_bytes: bytes, samples_per_symbol: int | float,
                              bit_map: dict[int, complex], bits_per_symbol: int) -> numpy.ndarray:
    """Convert a binary bytes object to an array given a mapping and generic specifications.
    ...
    Args:
        ...
        bit_map: The mapping of bits to complex samples.
        bits_per_symbol: The number of bits represented in one symbol (e.g., BFSK == 1, QPSK == 2,
            16-QAM == 4).
    """
    # LOCAL VARIABLES
    samples = None  # An array of the sample values
    ...
    # PREPARE
    bin_bytes = pad_bin_bytes(bin_bytes, bits_per_symbol)
