"""Unit test module for QAM16.demodulate().
...
Typical Usage:
    ...
    python -m test.unit_test.test_modem.test_qam16.test_modem_qam16_demodulate
    # Run just this normal 1 unit test
    python -m test.unit_test.test_modem.test_qam16.test_modem_qam16_demodulate -k n01
"""
...
class ModemQAM16DemodulateUnitTest(ModemQAM16UnitTest):
    """Parent class for all QAM16.demodulate() unit tests."""
    ...
    def run_test_exception_input(self, samples: Any, filt: Any, strategy: Any,
                                 exception_type: Exception, exception_msg: str) -> None:
        """Common method calls for a test case expected to raise an exception.

        Test author must call self.set_qam16_ctor_args().
        ...
        Args:
            ...
            exception_msg: A sub-string, empty or not, to look for in the raised Exception.
        """
        self.set_test_input(samples, filt, strategy)
    ...
    def run_test_return_compute(self, sample_rate: float, symbol_rate: float,
                                ...
                                mapper: dict[int, complex] | None, filt: MatchedFilter,
                                strategy: DecideSymbols, exp_ret: bytes) -> None:
        ...
        self.run_test_return_input(sample_rate=sample_rate, symbol_rate=symbol_rate,
                                   carrier_recovery=carrier_recovery, mapper=mapper,
                                   samples=test_in, filt=filt, strategy=strategy, exp_ret=exp_ret)

    def run_test_return_input(self, sample_rate: float, symbol_rate: float,
                              carrier_recovery: CostasLoop | None,
                              mapper: dict[int, complex] | None, samples: numpy.ndarray,
                              filt: MatchedFilter, strategy: DecideSymbols, exp_ret: bytes) -> None:
    ...
    def run_test_return_noisy(self, sample_rate: float, symbol_rate: float,
                              ...
                              filt: MatchedFilter, strategy: DecideSymbols) -> None:
        """Common method calls for a test case expected to return a computed result.
...
class NormalModemQAM16DemodulateUnitTest(ModemQAM16DemodulateUnitTest):
    """Normal Test Cases."""

    def test_n01_quad_word_random_bits(self):
        ...
        self.run_test_return_compute(samp_rate, sym_rate, carr_rec, mapper, filt, strategy, bits)

    def test_n02_quad_word_repeating_all_symbols(self):
        """Repeating all symbols for eight bytes."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
        mapper = QAM16_MAP  # Necessary to right-size the binary input
        # Setup
        repeat = int(8 * 8 / len(mapper))  # Number of repeats
        # QAM16().demodulate() input
    ...
    def test_n03_random_bits_strategy_near(self):
        """Random bits - decide symbols strategy: near."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
        mapper = None      # Defaults to QAM16_MAP
        # QAM16().demodulate() input
        bits = generate_bin_bytes(num_bits=1024)  # Expected ret value computes the 'samples' input
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.AXIS
        self.run_test_return_compute(samp_rate, sym_rate, carr_rec, mapper, filt, strategy, bits)

    def test_n04_random_bits_strategy_kmeans(self):
        """Random bits - decide symbols strategy: kmeans."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
    ...
    def test_n05_random_bits_strategy_kmeans_gain(self):
        """Random bits - decide symbols strategy: kmeans w/ gain."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
    ...
    def test_n06_random_bits_strategy_near(self):
        """Random bits - decide symbols strategy: near."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
    ...
    def test_n07_random_bits_with_good_snr(self):
    ...
    def test_n08_random_bits_with_marginal_snr(self):
        """Random bits with AWGN at a marginal SNR."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
...
class ErrorModemQAM16DemodulateUnitTest(ModemQAM16DemodulateUnitTest):
    """Error Test Cases."""
    ...
    def test_e13_bad_carrier_recover_type_dict(self):
        ...
        samp_rate = 48000
        sym_rate = 800
        carr_rec = QAM16_MAP  # Represents flipped config arguments
        mapper = None        # Defaults to QAM16_MAP
        # QAM16().demodulate() input
        samples = self.SAMPLES_OOK_ALL_10S  # Just to get a valid array
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.NEAR
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.run_test_exception_input(samples, filt, strategy, NotImplementedError,
...
class BoundaryModemQAM16DemodulateUnitTest(ModemQAM16DemodulateUnitTest):
    ...
    template_nsym_exc = 'Requires at least 16 symbols to cluster but received {}'
    ...
    def test_b07_inadequate_min_num_syms_one(self):
        """Minimum number of symbols not met: 1 (requires 16)."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
        mapper = QAM16_MAP  # Necessary to create the samples
        # QAM16().demodulate() input
    ...
    def test_b08_barely_inadequate_min_num_syms(self):
        """Minimum number of symbols not met: 15 (requires 16)."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
        mapper = QAM16_MAP  # Necessary to create the samples
        # QAM16().demodulate() input
        num_syms = 15      # The number of symbols to generate for this test case
        bits = generate_bin_bytes(num_bits=self.bits_per_symbol*num_syms)  # Source binary
        samples = convert_bin_bytes_to_qam16(bin_bytes=bits, sample_rate=samp_rate,
                                             symbol_rate=sym_rate, bit_map=mapper)
        filt = MatchedFilter.NONE
    ...
    def test_b09_barely_met_min_num_syms(self):
        """Barely met the minimum number of symbols: 16."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
        mapper = None      # Defaults to QAM16_MAP
        # QAM16().demodulate() input
        num_syms = 16      # The number of symbols to generate for this test case
    ...
    def test_b10_barely_exceeded_min_num_syms_five(self):
        """Barely exceeded the minimum number of symbols: 17 (requires at least 16)."""
        # QAM16Config() input
        samp_rate = 32000  # GNU Radio tutorial example settings
        sym_rate = 8000    # GNU Radio tutorial example settings
        carr_rec = None
        mapper = None      # Defaults to QAM16_MAP
        # QAM16().demodulate() input
        num_syms = 17      # The number of symbols to generate for this test case
        bits = generate_bin_bytes(num_bits=self.bits_per_symbol*num_syms)
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.NEAR  # Default value
        self.run_test_return_compute(samp_rate, sym_rate, carr_rec, mapper, filt, strategy, bits)

    def test_b11_double_word_random_bits(self):
        """Not enough symbols to use k-means clustering: four bytes of random bits."""
        # QAM16Config() input
        samp_rate = 32000   # GNU Radio tutorial example settings
        sym_rate = 8000     # GNU Radio tutorial example settings
        carr_rec = None
        ...
        self.set_qam16_ctor_args(samp_rate, sym_rate, carr_rec, mapper)
        self.run_test_exception_input(samples, filt, strategy, ValueError,
                                      'symbols to cluster but received')


class SpecialModemQAM16DemodulateUnitTest(ModemQAM16DemodulateUnitTest):
    """Special Test Cases."""
    ...
    def test_s20_one_symbol_repeated_zero(self):
        ...
        mapper = None       # Defaults to QAM16_MAP
        # QAM16().demodulate() input
        bits = b'0000' * 128  # Expected ret value computes the 'samples' input
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.NEAR  # Default value
        self.run_test_return_compute(samp_rate, sym_rate, carr_rec, mapper, filt, strategy, bits)
    ...
    def test_s21_one_symbol_repeated_one(self):
        ...
        mapper = None       # Defaults to QAM16_MAP
        # QAM16().demodulate() input
        bits = b'0001' * 128  # Expected ret value computes the 'samples' input
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.NEAR  # Default value
        self.run_test_return_compute(samp_rate, sym_rate, carr_rec, mapper, filt, strategy, bits)
    ...
    def test_s22_one_symbol_repeated_two(self):
        ...
        mapper = None       # Defaults to QAM16_MAP
        # QAM16().demodulate() input
        bits = b'0010' * 128  # Expected ret value computes the 'samples' input
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.NEAR  # Default value
        self.run_test_return_compute(samp_rate, sym_rate, carr_rec, mapper, filt, strategy, bits)
    ...
    def test_s23_one_symbol_repeated_three(self):
        ...
        mapper = None       # Defaults to QAM16_MAP
        # QAM16().demodulate() input
        bits = b'0011' * 128  # Expected ret value computes the 'samples' input
        filt = MatchedFilter.NONE
        strategy = DecideSymbols.NEAR  # Default value
        self.run_test_return_compute(samp_rate, sym_rate, carr_rec, mapper, filt, strategy, bits)
    ...
    def test_s24_odd_binary_len_leading_zero(self):
        ...
        strategy = DecideSymbols.NEAR  # Default value
        # Test case setup
        exp_ret = bits + b'0'  # QAM16 output is always even
        ...
        self.run_test_return_input(sample_rate=samp_rate, symbol_rate=sym_rate,
                                   ...
                                   samples=samples, filt=filt, strategy=strategy, exp_ret=exp_ret)
...
def create_noisy_test_input(sample_rate: int | float, symbol_rate: int | float,
    ...
    Args:
        ...
        bit_map: [OPTIONAL] The bit --> complex sample decisions for use with the modulation.
            If None, uses QAM16_MAP (see: gallant_input.modem.constants).
    """
    mapping = bit_map
    if mapping is None:
        mapping = QAM16_MAP
