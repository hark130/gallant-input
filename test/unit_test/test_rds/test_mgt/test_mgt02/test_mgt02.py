"""Defines the root rds.mgt RDSMsgGroupType02 unit test class.

RDSMsgGrpType02UnitTest is the parent class for all rds.mgt sub-package related unit test classes.
    ...
    class RDSSomethingUnitTest(RDSMsgGrpType02UnitTest):
...
class RDSMsgGrpType02UnitTest(RDSMsgGrpTypeUnitTest):
    """Parent class for all rds.mgt RDSMsgGroupType02 specific unit tests.
    ...
    Attributes:
        ...
        built_rds_mgt02:    # RDSMsgGroupType02() object for this test case
    ...
    def __init__(self, *args, **kwargs) -> None:
        """RDSMsgGrpType02UnitTest ctor."""
        # ATTRIBUTES
        self.built_rds_mgt02 = None  # RDSMsgGroupType02() object to test
    ...
    def get_rds_group_msg_group02(self, rds_group: bytes, assume_na: bool = True) -> None:
        """Build an RDSMsgGroupType02() object to test."""
        self.build_rds_group(rds_group=rds_group, assume_na=assume_na)
        try:
            self.built_rds_mgt02 = self.built_rds_group.get_msg_group02()
        except (RDSIntegrityFailure, RDSMsgGroupTypeMissing, TypeError, ValueError) as err:
            self.fail_test_case(f'RDSGroup({rds_group}, {assume_na}).get_msg_group02() failed to '
                                'build an RDSMsgGroupType02() object as test case input: '
                                f'{repr(err)}')
