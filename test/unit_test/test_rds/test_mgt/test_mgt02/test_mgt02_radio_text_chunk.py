"""Unit test module for RDSMsgGroupType02.radio_text_chunk property.
...
Typical Usage:
    ...
    python -m test.unit_test.test_rds.test_mgt.test_mgt02    # Run just the RDSMsgGroupType02 tests
    # Run just the RDSMsgGroupType02.radio_text_chunk test cases
    python -m test.unit_test.test_rds.test_mgt.test_mgt02.test_mgt02_radio_text_chunk
"""
...
class RDSMsgGrpType02RTCUnitTest(RDSMsgGrpType02UnitTest):
    ...
    def call_callable(self):
        ...
        self.get_rds_group_msg_group02(rds_group=self.input_rds_group,
                                       assume_na=self.input_assume_na)
        return self.built_rds_mgt02.radio_text_chunk
...
# Leave me be, Pylint.
class NormalRDSMsgGrpType02RTCUnitTest(RDSMsgGrpType02RTCUnitTest):
    """Normal Test Cases."""

    def test_n02_good_group2_offset_0(self):
        """Live capture of a coherent RDS Group 2: Radio Text Offset 0."""
        rds_group = self.GOOD_GROUP4_MSG02_OFF0
        assume_na = True
        exp_ret = 'KONO'
        self.run_test_return(rds_group, assume_na, exp_ret)

    def test_n03_good_group2_offset_1(self):
        """Live capture of a coherent RDS Group 2: Radio Text Offset 1."""
        rds_group = self.GOOD_GROUP5_MSG02_OFF1
        assume_na = True
        exp_ret = ' 101'
        self.run_test_return(rds_group, assume_na, exp_ret)
