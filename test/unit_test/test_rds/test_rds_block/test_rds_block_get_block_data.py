"""Unit test module for RDSBlock.get_block_data().
...
Typical Usage:
    ...
    python -m test.unit_test.test_rds.test_rds_block.test_rds_block_get_block_data
    # Run just this normal 1 unit test
    python -m test.unit_test.test_rds.test_rds_block.test_rds_block_get_block_data -k n01
"""
...
class RDSBlockGBDUnitTest(RDSBlockUnitTest):
    """Parent class for all RDSBlock.get_block_data() unit tests."""
    ...
    def call_callable(self):
        ...
        return test_obj.get_block_data(*self._args, **self._kwargs)
    ...
    def validate_return_value(self, return_value):
        ...
        """
        # LOCAL VARIABLES
        exp_len = len(self._exp_return)  # Length of the expected value
        act_len = 0                      # Length of the actual return value

        # VALIDATE IT
        self._validate_return_value(return_value=return_value)
        # Length
        try:
            act_len = len(return_value)
            if act_len != exp_len:
                self._add_test_failure(f'Expected value length "{exp_len}" '
                                       f'but received a length of "{act_len}" instead')
        except TypeError:
            pass  # Any "TypeError: object of type '*' has no len()" exception already failed above
    ...
    def run_test_return(self, rds_block: bytes, block_id: BlockID) -> None:
        ...
        self.set_ctor_args(rds_block=rds_block, block_id=block_id)
        self.set_test_input()  # This method does not take any args
        self.expect_return(rds_block[:16])  # Data field is the first 16 bits of the RDS block
        self.run_test()


class NormalRDSBlockGBDUnitTest(RDSBlockGBDUnitTest):
    """Normal Test Cases."""
...
class ErrorRDSBlockGBDUnitTest(RDSBlockGBDUnitTest):
    ...
    def test_e04_rds_block_bad_value_empty(self):
        ...
        block_id = BlockID.GUESS
        self.run_test_exception(rds_block, block_id, ValueError, 'The "rds_block" argument must '
                                'be of length "26" instead of')
...
class SpecialRDSBlockGBDUnitTest(RDSBlockGBDUnitTest):
    """Special Test Cases."""
