"""Unit test module for RDSPICode.get_station_name().
...
Typical Usage:
    ...
    python -m test.unit_test.test_rds.test_rds_pi_code.test_rds_pi_code_get_station_name
    # Run just this normal 1 unit test
    python -m test.unit_test.test_rds.test_rds_pi_code.test_rds_pi_code_get_station_name -k n01
"""
...
class RDSPICodeGSNUnitTest(RDSPICodeUnitTest):
    """Parent class for all RDSPICode.get_station_name() unit tests.
    ...
    def call_callable(self):
        ...
        return self.test_obj.get_station_name(*self._args, **self._kwargs)
...
class NormalRDSPICodeGSNUnitTest(RDSPICodeGSNUnitTest):
    """Normal Test Cases."""

    def test_n01_valid_complete_station_name_set(self):
        """Valid and complete set of all station name offsets."""
        stream_bytes = self.GOOD_SET1_MSG00A
        pi_code = stream_bytes[:RDS_BLOCK_DATA_LEN]
        exp_ret = self.GOOD_SET1_STATION_NAME
        self.run_test_return(pi_code, stream_bytes, exp_ret)

    def test_n02_valid_complete_station_name_set_plus_some(self):
        """Valid and complete set of all radio text offsets."""
        stream_bytes = self.GOOD_SET1_MSG00A + self.GOOD_SET2_MSG02A
        pi_code = stream_bytes[:RDS_BLOCK_DATA_LEN]
        exp_ret = self.GOOD_SET1_STATION_NAME
        self.run_test_return(pi_code, stream_bytes, exp_ret)

    def test_n03_no_station_name(self):
        """Valid RDS groups for this PI Code, but no Group Type 00s.
...
class BoundaryRDSPICodeGSNUnitTest(RDSPICodeGSNUnitTest):
    """Boundary Test Cases."""
    ...
    def test_b01_double_valid_complete_radio_text_sets(self):
        ...
        num_captured = 2
        stream_bytes = self.GOOD_SET1_MSG00A * num_captured
        pi_code = stream_bytes[:RDS_BLOCK_DATA_LEN]
        exp_ret = self.GOOD_SET1_STATION_NAME * num_captured
        self.run_test_return(pi_code, stream_bytes, exp_ret)
...
class SpecialRDSPICodeGSNUnitTest(RDSPICodeGSNUnitTest):
    """Special Test Cases."""
    ...
    def test_s02_missing_trailing_offsets(self):
        ...
        missing = 2  # The number of RDS groups to remove
        stream_bytes = self.GOOD_SET1_MSG00A[:len(self.GOOD_SET1_MSG00A)-missing * RDS_GROUP_LEN]
        exp_ret = self.GOOD_SET1_STATION_NAME[:len(self.GOOD_SET1_STATION_NAME)-missing*2]
        pi_code = stream_bytes[:RDS_BLOCK_DATA_LEN]
        self.run_test_return(pi_code, stream_bytes, exp_ret)

    def test_s03_missing_leading_offsets(self):
        """Valid, ordered, radio text offsets missing a few leading offsets."""
        missing = 1  # The number of RDS groups to remove
        stream_bytes = self.GOOD_SET1_MSG00A[missing * RDS_GROUP_LEN:]
        exp_ret = '?' * missing * 2 + self.GOOD_SET1_STATION_NAME[missing*2:]
        pi_code = stream_bytes[:RDS_BLOCK_DATA_LEN]
        self.run_test_return(pi_code, stream_bytes, exp_ret)
    ...
    def test_s05_missing_bookend_offsets(self):
        ...
        start_grp = 1  # The starting group to keep
        keeping = 2    # The number of RDS groups to remove
        stream_bytes = self.GOOD_SET1_MSG00A[start_grp * RDS_GROUP_LEN:
                                             (start_grp+keeping)*RDS_GROUP_LEN]
        exp_ret = '?' * start_grp * 2 \
                  + self.GOOD_SET1_STATION_NAME[start_grp*2:(start_grp+keeping)*2]
        pi_code = stream_bytes[:RDS_BLOCK_DATA_LEN]
        self.run_test_return(pi_code, stream_bytes, exp_ret)
    ...
    def test_s06_out_of_order_offsets(self):
        ...
        stream_bytes = self.GOOD_SET1_GRP01_MSG00_OFF00 + self.GOOD_SET1_GRP02_MSG00_OFF01 \
            + self.GOOD_SET1_GRP03_MSG00_OFF02 + self.GOOD_SET1_GRP02_MSG00_OFF01
        exp_ret = 'KO' + 'NO' + '  ' + '??NO'
        pi_code = stream_bytes[:RDS_BLOCK_DATA_LEN]
        self.run_test_return(pi_code, stream_bytes, exp_ret)
